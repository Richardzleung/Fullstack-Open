...
const parseSickLeave = (sickLeave: any): SickLeave | undefined => {
  ...
  if (!isSickLeave(sickLeave)) {
    throw new Error(
      `Invalid sick leave date(s)}`
    );
  }
  return sickLeave;
};

const parseHealthCheck = (object: HealthCheckInput): HealthCheckInput => {
...
const parseHospital = (object: HospitalInput): HospitalInput => {
...
const parseOccupationalHealthcare = (object: OccupationalHealthcareInput): OccupationalHealthcareInput => {
...
function isType(object: any): object is EntryEntry{
 return (object as EntryEntry).type !== undefined;
} 

// eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
const toNewDiagnosisEntry = (object: any): EntryEntry | Error=> {
  if (isType(object)){
    switch (object.type){
      case "HealthCheck":
